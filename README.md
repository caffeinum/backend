...
Website consists of three modules:

- frontend in justmedia repo
- this backend + nginx + mongodb
- python crawlers filling up the db

#### frontend

The most easy. Just clone and run
...
It uses React, Redux, Webpack and builds a static site into `build` dir.
...
# 2. setup website
ln -s ./morejustmedia.conf /etc/nginx/sites-available/morejustmedia
ln -s /etc/nginx/sites-available/morejustmedia /etc/nginx/sites-enabled/morejustmedia
sudo service nginx restart

# 3a. install mongodb
sudo apt install mongodb-server

# 3b. or connect to existing instance via port forwarding to localhost
ssh $IP -L 27017:localhost:27017 -H -f
...
npm run prod
# or
npx pm2 start --name "justmedia" "npm run prod"
```
...
Below is more detailed documentation for this backend:
